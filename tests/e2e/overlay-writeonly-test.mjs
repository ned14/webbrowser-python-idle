#!/usr/bin/env node
// Inverted sanitize: keep ONLY non-image-matching records (guest WRITE
// deltas), delete the read-through cache records. If warm boots then
// succeed, the read-cache records (created by the overlay's read path) are
// what poisons boots, and a fix that skips read-caching would work.
...
const res = await page.evaluate(async (imgHashes) => {
	...
	for (const k of keys) {
		...
		if (h1 === ih[0] && h2 === ih[1]) delKeys.push(k);
		else kept++;
	}
	if (delKeys.length) {
...
console.log('read-cache records deleted:', JSON.stringify(res));
...
console.log(`\n=== write-records-only overlay: ${okc}/${ITER} OK ===`);
await browser.close();